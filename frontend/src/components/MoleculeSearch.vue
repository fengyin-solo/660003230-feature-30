  ...
  <div class="bg-slate-800 rounded-lg p-4 border border-slate-700">
    ...
    <div class="space-y-1 max-h-96 overflow-y-auto">
      <div v-for="mol in store.filteredMolecules" :key="mol.id" @click="store.selectMolecule(mol)"
        :class="['cursor-pointer p-2 rounded-lg border transition-all', store.benchmarkMolecule?.id === mol.id ? 'border-orange-500' : store.currentMolecule?.id === mol.id ? 'border-cyan-500 bg-cyan-900/30' : 'border-slate-700 bg-slate-900 hover:border-slate-500']">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-1">
            ...
            <button
              @click.stop="store.toggleBenchmark(mol)"
              :class="['text-xs px-1.5 py-0.5 rounded transition-colors', store.benchmarkMolecule?.id === mol.id ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-slate-600 text-slate-200 hover:bg-slate-500']">
              {{ store.benchmarkMolecule?.id === mol.id ? '取消基准' : '设为基准' }}
            </button>
          </div>
        </div>
